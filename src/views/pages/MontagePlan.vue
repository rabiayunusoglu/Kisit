		...
		<CCard>
			...
			<CCardBody>
				<CDataTable
					...
					:items-per-page="5"
					:itemsPerPageSelect="{ label: 'Sayfalar' }"
					:tableFilter="{ label: 'Filtre', placeholder: 'Ara' }"
					:noItemsView="{ noResults: 'Sonuç Yok', noItems: 'Tablo Boş' }"
					hover
					sorter
					pagination
				/>
...
export default {
	...
	methods: {
		fetchData() {
			axios
				.get(`${CONFIG.api.invokeUrl}montageProductPlan`,{headers: {'Authorization': `Basic ${ServiceToken.getToken()}`}})
				.then(response => {
					 var i=0;
